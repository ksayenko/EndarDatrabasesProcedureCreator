...
namespace CreateUploadEDD
{
    ...
    public class DataAccess : IDisposable
    {
        ...
        {
            string sql = "EXEC sp_helptext '" + name + "';";
            return Select(sql, "sp_helptext");
        }

        public DataTable Edd_Process_Columns(string tablename)
        {

            string sql = "SELECT ";
            sql += " COL.TABLE_NAME AS [PROD_TABLE]";
            sql += ", COL.COLUMN_NAME AS [PROD_COLUMN]";
            sql += ", COL.IS_NULLABLE AS [PROD_COL_NULLABLE]";
            sql += ", COL.DATA_TYPE AS [PROD_COL_TYPE]";
            sql += ", COL.CHARACTER_MAXIMUM_LENGTH AS [PROD_COL_LEN]";
            sql += ", COL.ORDINAL_POSITION as [POSITION]";

            sql += ", COL_UPLOAD.TABLE_NAME AS [UPLOAD_TABLE]";
            sql += ", COL_UPLOAD.COLUMN_NAME AS [UPLOAD_COLUMN]";
            sql += ", COL_UPLOAD.DATA_TYPE AS [UPLOAD_COL_TYPE]";
            sql += ", COL_UPLOAD.CHARACTER_MAXIMUM_LENGTH AS [UPLOAD_COL_LEN]";
            sql += ", COL_UPLOAD_ARCHIVE.TABLE_NAME AS [UPLOAD_ARCH_TABLE]";
            sql += ", COL_UPLOAD_ARCHIVE.COLUMN_NAME AS [UPLOAD_ARCH_COLUMN]";
            sql += ", COL_UPLOAD_ARCHIVE.DATA_TYPE AS [UPLOAD_ARCH_COL_TYPE]";
            sql += ", COL_UPLOAD_ARCHIVE.CHARACTER_MAXIMUM_LENGTH AS [UPLOAD_ARCH_COL_LEN]";
            ...
            sql += " AND	TBL.TABLE_TYPE = 'BASE TABLE'";
            sql += " LEFT JOIN INFORMATION_SCHEMA.COLUMNS AS COL_UPLOAD";
            sql += " ON	COL_UPLOAD.TABLE_NAME = 'UPLOAD_' + COL.TABLE_NAME";
            sql += " AND	COL_UPLOAD.COLUMN_NAME = COL.COLUMN_NAME";
            sql += " LEFT JOIN INFORMATION_SCHEMA.COLUMNS AS COL_UPLOAD_ARCHIVE";
            sql += " ON	COL_UPLOAD_ARCHIVE.TABLE_NAME = 'UPLOAD_' + COL.TABLE_NAME + '_ARCHIVE'";
            sql += " AND	COL_UPLOAD_ARCHIVE.COLUMN_NAME = COL.COLUMN_NAME";
            sql += " WHERE COL.TABLE_NAME NOT LIKE 'UPLOAD_%'";
            sql += " AND COL.TABLE_NAME NOT LIKE 'NOTIF_%'";
            sql += " AND COL.TABLE_NAME NOT LIKE 'SEC_%'";
            sql += " AND COL.TABLE_NAME NOT IN (";
            sql += " 'TEMP_GROUNDWATER_READING',";
            sql += " 'ERROR_LOG',";
            sql += " 'UPLOAD_REQUEST',";
            sql += " 'UPLOAD_RESPONSE',";
            sql += " 'FILE_UPLOAD',";
            sql += " 'LOG_ACTIVITY',";
            sql += " 'USERS',";
            sql += " 'LOC_LOCATION_ALIAS'			)";
            sql += " AND COL.COLUMN_NAME NOT LIKE '_LAST_%'";
            sql += " AND COL.COLUMN_NAME NOT LIKE 'SAMPLE_RECORD_ID' AND COL.COLUMN_NAME NOT LIKE 'TEST_RECORD_ID'";
            if (tablename != "")
                sql += " and COL.TABLE_NAME = '" + tablename + "';";

            return Select(sql, "Edd_Process_Columns");

        }

        public DataTable Edd_Process_Tables()
        {
            ...
            return Select(sql, "Edd_Process_Tables");
